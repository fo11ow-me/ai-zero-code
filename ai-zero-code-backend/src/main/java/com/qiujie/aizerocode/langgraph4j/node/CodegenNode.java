package com.qiujie.aizerocode.langgraph4j.node;

import com.qiujie.aizerocode.constant.AppConstant;
import com.qiujie.aizerocode.core.AiCodegenFacade;
import com.qiujie.aizerocode.langgraph4j.model.QualityResult;
...
public class CodegenNode {
    ...
    public static AsyncNodeAction<MessagesState<String>> create() {
        return node_async(state -> {
            ...
            // 获取 AI 代码生成外观服务
            AiCodegenFacade aiCodegenFacade = SpringContextUtil.getBean(AiCodegenFacade.class);
            log.info("开始生成代码，类型: {} ({})", codeGenTypeEnum.getValue(), codeGenTypeEnum.getText());
            // 先使用固定的 appId (后续再整合到业务中)
            Long appId = 0L;
            // 调用流式代码生成
            Flux<String> codeStream = aiCodegenFacade.generateAndSaveCodeStream(userMessage, codeGenTypeEnum, appId);
            // 同步等待流式输出完成
            codeStream.blockLast(Duration.ofMinutes(10)); // 最多等待 10 分钟
            // 根据类型设置生成目录
            String generatedCodeDir = String.format("%s/%s/%s", AppConstant.CODE_SAVE_PATH, codeGenTypeEnum.getValue(), appId);
